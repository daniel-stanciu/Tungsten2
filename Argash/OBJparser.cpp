...
void OBJparser::Parse(WCHAR* path,WCHAR* outputPath,int orientation)
{
	...
	*/
	...
	faceCount=(int)sizeof(SimpleVertex)*vertexData.size();
	WriteFile(output,&faceCount,sizeof(faceCount),NULL, NULL);
...
void OBJparser::Draw(Model& model)
{
	Services.Graphics()->SetConstantBufferVS(MemoryBlock(&model.worldMatrix,sizeof(XMMATRIX)),1);
	Services.Graphics()->SetBuffers(model.vertex,model.index);
	Services.Graphics()->DrawIndexed(model.index.width/sizeof(UINT32),0,0);
}

Model OBJparser::Load(WCHAR* path)
{
...
int OBJparser::CheckOrientation(SimpleVertex* data)
{
	int handedness=0;
	XMVECTOR vector[2];
